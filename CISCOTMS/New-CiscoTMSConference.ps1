...
#####################################################################
#
# Värden för användare, sökväg till pwd-fil samt sökväg till API
#
#####################################################################
...
$PostRequest = (Invoke-WebRequest -Uri $url -InFile C:\GitHub\Powershell_script\CISCOTMS\original_GetDefaultConference.xml -ContentType 'text/xml' -Method POST -Credential $credential)
