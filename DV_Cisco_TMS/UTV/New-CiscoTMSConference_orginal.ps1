...
(
	...
	[string]$BookedBy,
	[Parameter(Mandatory = $true, HelpMessage = 'The particpants')]
	[array]$ExtDeltagare,
	[Parameter(Mandatory = $true, HelpMessage = 'If the scripts runs as test or production')]
	[ValidateSet('Yes', 'No', IgnoreCase = $true)]
	[string]$TestOnly = 'Yes',
  [Parameter(Mandatory = $True, HelpMessage = 'If the meeting should be secure or not')]
	[ValidateSet('Yes', 'No', IgnoreCase = $true)]
	[string]$SecureMeeting = 'No'
)
...
$JoinUrlSecure=$config.configtms.JoinUrl
...
<b><a href="$JoinUrlSecure">Länk att ansluta till.</a></b>
